...
public class basketBallController  implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        try {
            ...
            ResultSet qResult = statement.executeQuery("SELECT * FROM `kosarLabda`");
            int k = 0;
            while (qResult.next()) {
                if(k < 10 && a < 1) {
                    ...
                    vOdds.add(qResult.getString(6));
                    fullId.add(qResult.getString(7));
                }
                k++;
            }
            a++;
    ...
    public void listClick() {
        ...
        try {
            ...
            String insertTo = "INSERT INTO aktualis(id, liga, hazaiCsapat, vendegCsapat, hazaiOdds, dontetlenOdds, vendegOdds, fullID) VALUES (1, '" +
                    liga.get(hmm-1) + "','" + hazaiCs.get(hmm-1) + "','"  + vendegCs.get(hmm-1) + "','" + hOdds.get(hmm-1) + "',' 0 ','" + vOdds.get(hmm-1)+ "','" + fullId.get(hmm-1) + "')";
            Statement statement = connectDB.createStatement();
            ...
            Parent root = FXMLLoader.load(getClass().getResource("/fxml/merkozesKosar.fxml"));
